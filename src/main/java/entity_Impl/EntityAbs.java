...
//        int tileSize = m_screenService.getTileSize();
//        if (Math.abs(otherPosition.x - myPosition.x) < tileSize / m_blockSegments && Math.abs(otherPosition.y - myPosition.y) < tileSize / m_blockSegments) {
//            int a = 3;
//        }
...
//        // Since we are working with grid movement, if x and y are both close, either both x's or both y's are equal.
        return getGridPosition().equals(otherEntity.getGridPosition());
    }
